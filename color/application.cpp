...
// we use an embedded unnamed namespace for static linkage.
...
const char* window_title = "OpenGL GPGPU Color Shader";

// Geometry Data for the 2D Screen
const struct { float x, y; } vertices[] = {{-1, -1}, {1, -1}, {1, 1}, {-1, 1}};
const struct { uint32_t id[3]; } triangles[] = {{0, 1, 2}, {0, 2, 3}};
...
const char* fragment_shader_text =
    ...
    "in vec2 color;"
    "void main(){"
    "  float red = 0.5 * color.x + 0.5 * color.y;"
    "  float green = 0 * color.x + 1.0 * color.y;"
    "  float blue = 1.0 * color.x + 0 * color.y;"
...
void init() {
  ...
  is_initialized = true;
  cout << "Created OpenGL test application without errors!\n"
...
void render() {
  ...
  glDrawElements(GL_TRIANGLES, sizeof(triangles) / sizeof(triangles[0].id[0]),
                 GL_UNSIGNED_INT, 0);
}
